-- Macedonia profile About field extension
-- Run once in the Supabase SQL Editor for an existing Macedonia database.
-- Any prior profile column is intentionally left untouched so no pre-existing
-- profile information is discarded; new registrations use about.
...
begin
  ...
  insert into public.profiles (
    id, email, full_name, phone, about, referral_code, referred_by
  ) values (
    new.id,
    coalesce(new.email, ''),
